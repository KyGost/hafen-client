...
public class MapView extends PView implements DTarget, Console.Directory {
	...
	public class FreeCam extends Camera {
		...
		public void rotate(Coord r) {
			...
			if (telev > (Math.PI / 2.0)) {
				telev = (float)Math.PI / 2.0f;
			}
	...
	private void amblight() {
		synchronized (glob) {
			if (glob.lightamb != null) {
				// TODO: Rip this shit out; replace with constant lighting
	...
	public abstract class Hittest {
		...
		private void ckdone(int fl) {
			...
			if (done) {
				synchronized (ui) {
					if (mapcl != null) {
						if (Config.center_tile) {
							mapcl = mapcl.floor(tilesz).mul(tilesz).add(5, 5);
						}
...
// TODO: All the below better be used by bots. Ender was just else {inspect(c);}
