...
public class OptWnd extends BetterWindow {
	...
	public OptWnd(boolean gopts) {
		...
		addPanelButton("Combat", 'b', combat, colum, s row++);
	...
	private void initGeneralPanel(Panel panel) {
		...
		panel.add(new Dropbox <String>(UI.scale(80), 5, UI.scale(16)) {
			...
			public void change(String item) {
				super.change(item);
				if (!item.equals(L10N.LANGUAGE.get())) {
					L10N.LANGUAGE.set(item);
				}
			}
		...
		panel.add(new Dropbox <UI.KeyMod>(UI.scale(100), 5, UI.scale(16)) {
			...
			public void change(UI.KeyMod item) {
				super.change(item);
				if (!item.equals(CFG.MENU_SKIP_AUTO_CHOOSE.get())) {
					CFG.MENU_SKIP_AUTO_CHOOSE.set(item, true);
				}
			}
