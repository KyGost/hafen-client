...
public class CharWnd extends BetterWindow {
	...
	public static class Constipations extends Listbox <Constipations.El> {
		...
		public class El {
			...
			String tip() {
				return(name != null ? String.format("%s %d%%", name, ((int)(100 * (1 - a)))) : null);
			}
	...
	public static class StudyInfo extends Widget {
		...
		public Coord contentsz() {
			return(super.contentsz().add(UI.scale(5, 2)));
		}
				...
				if (msg == "conds") {
					...
					while (a < args.length) {
						...
						if (cond != null) {
							boolean ch = false;
							if (st != cond.done) {
								cond.done = st; ch = true;
							}
							if (!Utils.eq(status, cond.status)) {
								cond.status = status; ch = true;
							}
